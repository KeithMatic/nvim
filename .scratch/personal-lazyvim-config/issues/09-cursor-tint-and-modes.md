...
- [ ] The tint is applied from the theme-change hook, so it survives theme switches
- [ ] modes.nvim installed; mode colours derive from the active theme's palette
- [ ] Test: after `:Tint #ff0000 0.3`, CursorLine, the explorer cursor-line group and the completion-selection group all have the expected blended background
- [ ] Test: the tint persists after booting a second headless instance
- [ ] Test: the tint persists after switching theme (blended against the new theme's background)
- [ ] Test: invalid hex or an out-of-range fade produces an error and leaves the highlights unchanged
- [ ] A sensible default tint on first launch
